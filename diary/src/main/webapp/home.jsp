...
<body>
	...
	<table class="table table-striped">
		...
		%>
				<tr>
					<td>
						<a href="./notice/noticeOne.jsp?noticeNo=<%=n.noticeNo%>">
						<%=n.noticeTitle%>
						</a>
					</td>
					<td><%=n.createdate%></td>
	...
	<table class="table">
		...
		%>
				<tr style="background-color: <%=s.scheduleColor%>;">
					...
					<td>
						<a href="./schedule/scheduleOne.jsp?scheduleNo=<%=s.scheduleNo%>">
						<%=s.scheduleMemo%>
						</a>
					</td>
				</tr>
	...
	<table class="table table-striped">
		...
		<tr>
			<td>
				...
				Calendar API를 이용하여 달력을 만들고 DB를 연동해서 DB에 데이터 입력과 데이터 수정, 데이터 출력, 데이터 삭제를 실습함.<br>
				JSP를 이용해서 action 페이지와 form 페이지를 오가며 데이터 출력을 위한 java코드를 짬.<br>
				추가적으로 게시판 페이지 10페이지씩 나누는 것과 달력에서 전달과 다음달의 날짜까지 출력하는것을 함.
			</td>
		</tr>
	</table>
</body>
